...
NTSTATUS SysMonDeviceControl(PDEVICE_OBJECT, PIRP Irp) {
	...
	case IOCTL_TERMINATE_PROCESS: {
		auto pid = *(ULONG*)buffer;
		if (stack->Parameters.DeviceIoControl.InputBufferLength > 0) {
			PEPROCESS process;
			status = PsLookupProcessByProcessId((HANDLE)pid, &process);
			if (NT_SUCCESS(status)) {
				KdPrint((DRIVER_PREFIX "Terminating process with PID=%u\n", pid));
				...
				KdPrint((DRIVER_PREFIX "Failed to find process with PID=%u\n", pid));
			}
		}
		else {
		...
		break;
								...
								//case IOCTL_BLOCK_PROGRAM: {
								//	if (stack->Parameters.DeviceIoControl.InputBufferLength > 0) {
								//		char msg[256] = { 0 };
								//		// Sao chép chuỗi từ buffer (UM) vào msg
								//		SIZE_T copyLength = min(stack->Parameters.DeviceIoControl.InputBufferLength, sizeof(msg) - 1);
