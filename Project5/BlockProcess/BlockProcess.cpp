...
void DisplayInfo(BYTE* buffer, DWORD size) {
	while (size > 0) {
		auto header = (ConnectionInfo*)buffer;
		DisplayTime(header->Timestamp);
		std::wstring processPath(header->ProcessPath, header->ProcessPathLength);
		UINT32 remoteAddr = header->RemoteAddress;
		UINT32 localAddr = header->LocalAddress;
		UINT16 remotePort = header->RemotePort;
		UINT16 localPort = header->LocalPort;

		// Format the log
		printf("[%s][IPV4] %s %u %u.%u.%u.%u:%u - %u.%u.%u.%u:%u\n",
			"IN",
			processPath.c_str(),  // Image path
			header->pid,
